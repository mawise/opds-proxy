...
func TestParseOpenSearchTemplate_PrefersAcquisition(t *testing.T) {
	xml := `<?xml version="1.0" encoding="UTF-8"?>
<OpenSearchDescription>
  <Url type="application/atom+xml" template="https://example.test/search?q={searchTerms}"/>
  <Url type="application/atom+xml;profile=opds-catalog" template="https://example.test/opds?q={searchTerms}"/>
  <Url type="application/atom+xml;profile=opds-catalog;kind=acquisition" template="https://example.test/acquisition?q={searchTerms}"/>
</OpenSearchDescription>`
	...
	want := "https://example.test/acquisition?q={searchTerms}"
	if tmpl != want {
		t.Fatalf("unexpected template. got %q want %q", tmpl, want)
	}
}

func TestParseOpenSearchTemplate_FallbackOther(t *testing.T) {
	xml := `<?xml version="1.0" encoding="UTF-8"?>
<OpenSearchDescription>
  ...
  <Url type="application/json" template="https://example.test/api?q={searchTerms}"/>
	...
	want := "https://example.test/html?q={searchTerms}"
	if tmpl != want {
		t.Fatalf("unexpected template. got %q want %q", tmpl, want)
	}
}
